...
let view_y_min = -1.3; // [m]
let view_y_max = 1.3; // [m]
...
function createInitParticles() {
	const tank_width = 0.6; // 容器の幅 [m]
	const tank_height = 1; // 容器の高さ [m]
	const liquid_width = 0.6; // 液柱の幅 [m]
	const liquid_height = 0.2; // 液柱の高さ [m]
	// 可視化領域
	view_x_min = 0.0;
	...
	// 	y_center=0.55
	// 	radius=0.1
	//     for (let ny = 1; ny < numY; ++ny) {
	//         for (let nx = 1; nx < numX; ++nx) {
	//         const x = (posX_max - posX_min) * (nx + 0.5) / numX + posX_min;
	//         const y = (posY_max - posY_min) * (ny + 0.5) / numY + posY_min;
	...
	let numR;
	let numTheta;
	numR = Math.floor((radius) / ell0 + 0.5);
	for (let nr = 0; nr <= numR; ++nr) {
		const r = nr * ell0;
		numTheta = Math.floor((2 * Math.PI * r / ell0))
		for (let ntheta = 0; ntheta < numTheta; ntheta++) {
			const theta = 2 * Math.PI * ntheta / numTheta;
			x = r * Math.cos(theta) + x_center;
			y = r * Math.sin(theta) + y_center;
			ret.push({
				type: type_solid,
				x: [x, y],
				v: [r / Radius * velocity * Math.cos(Math.atan((x - X_center) / (y - Y_center)) + Math.PI / 2), -r / Radius * velocity * Math.sin(theta)],
				r: [Math.sqrt((x - X_center) ** 2 + (y - Y_center) ** 2), Math.atan((x - X_center) / (y - Y_center)) + Math.PI / 2],
				p: 0.0,
			});
		}
	}
...
function update() {
	...
	for (let i = 0; i < particles.length; i++) {
		if (particles[i].type >= type_solid) {
			continue;
		}
		const x_i = particles[i].x[0];
		const y_i = particles[i].x[1];
	...
	for (let i = 0; i < particles.length; i++) {
		if (particles[i].type == type_solid) {
			const r = particles[i].r[0];
			particles[i].r[1] -= omega * dt;
			const theta = particles[i].r[1];
			particles[i].x[0] = r * Math.cos(theta) + X_center;
			particles[i].x[1] = r * Math.sin(theta) + Y_center;
			particles[i].v[0] = r / Radius * velocity * Math.sin(theta);
			particles[i].v[1] = -r / Radius * velocity * Math.cos(theta);
			continue;
		}
		if (particles[i].type >= type_wall) {
			continue;
...
//描画用：液体粒子の速度を返す関数
function convertVelocitiesToVectors(particles) {
	const vectors = [];
	for (let i = 0; i < particles.length; i++) {
		if (particles[i].type > type_fluid) continue;
		vectors.push((particles[i].v[0]), (particles[i].v[1]), 0);
	}
	return vectors;
}
...
function convertParticlesVeclocityToColors(particles) {
	...
	for (let i = 0; i < particles.length; i++) {
		...
		} else if (particles[i].type == type_solid) {
